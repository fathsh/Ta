...
class SubTask_insert_code(TaTask):
    ...
    def data_pre_treated(self,data):
        def convert_to_date(data):
            return xlrd.xldate.xldate_as_datetime(float(data), 0).strftime("%Y-%m-%d")
# =================================================================
        for x in data:
...
# ================================================================
    ...
    def set_value_frame_tab_119_161(self,data):
        for i in range(2):
            ...
            excel_data=data['清算天数设置_托管行清算天数设置' if i==0 else '清算天数设置_销售商清算天数设置']
