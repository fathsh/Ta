...
class SubTask_insert_code(TaTask):
    ...
    def __init__(self):
        ...
        self.excel_datas={}
        self.onchange_key = ['基金代码','销售商最大折扣']
    ...
    def get_new_code(self):
        ...
        lis_gupiao=self.super_find_eles('#id_1_0>li',frames='frame-tab-132',return_all=True)       #获取私募-股票下的所有li元素
        lis_qiquan=self.super_find_eles('#id_1_7>li',return_all=True)                               #获取私募-期权下的所有li元素
        t=self.skip_Exception(lambda :sorted([x.get_attribute('innerText') for x in lis_qiquan+lis_gupiao])[-1].split('：')[0])     #获取最后一个代码
    ...
    def compare_after_copy_code(self):
        ...
        data_excel=self.excel_datas['基金信息'][0]
    ...
    def set_value_after_compare(self):
        ...
        data={key:value[0] for key,value in self.result_compare.items() if key!='基金代码'}
        self.set_values(eles,data,'set_value_after_compare')
        self.compare_after_copy_code()
        if not self.result_compare or (len(self.result_compare)==1 and self.result_compare['基金代码']):
            ...
            self.form_submit(self.super_find_eles('#finish',frames='frame-tab-sysinfo_fundInfo-add-fund'),
                             frames_label=['frame-tab-sysinfo_fundInfo-add-fund','sysinfo_fundInfoBase-frame'])
            return
        else:
